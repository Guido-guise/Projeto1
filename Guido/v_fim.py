...
AREA_MINIMA = 15
SAIDA = "D:/Visao/Projeto1/Guido/saida_contagem"
...
for k in range(1,11):
    ...
    img0 = cv2.imread(CAMINHO_IMAGEM.format(k), cv2.IMREAD_COLOR)
    ...
    altura = img0.shape[0]
    img_recortada = img0[: int(altura * CORTE_ALTURA), :]
    ...
    resultados.append({'Img': k,'Altura Vert.': altura_orig,'Compr Total': comp_orig,'Diâmetro': diametro_orig,'Área': area,'Nro Folhas': estimado})
...
df_prev=df.copy()
df_real = pd.read_excel("Projeto1/Conjunto_VALIDACAO/Resultados_VALIDACAO.xlsx", header=1)
...
# remover coluna chave antes do MAPE
df_real_val = df_real.drop(columns=["Img"])
df_prev_val = df_prev.drop(columns=["Img"])

df_real_val = df_real_val.apply(pd.to_numeric, errors='coerce')
df_prev_val = df_prev_val.apply(pd.to_numeric, errors='coerce')

real_safe = df_real_val.replace(0, np.nan)

mape_por_coluna = (np.abs((df_real_val - df_prev_val) / real_safe)).mean() * 100

print("MAPE por coluna:")
print(mape_por_coluna)

print("\nMAPE total:")
print(mape_por_coluna.mean())
print("\n=== Tabela final ===")
