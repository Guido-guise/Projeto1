...
err=[]
valores=[62484,217423,179931,43952,28524]

for i in range(1, 6):
    img_1 = cv2.imread(f"Projeto1/_Eucalipto_Escolhidos2/Eucalipto{i}.jpg", cv2.IMREAD_COLOR)
    ...
    erro_rel=((valores[i-1]-area_foliar_pixels)/valores[i-1])*100
    print(f"Erro relativo: {int(erro_rel)}")
    err.append(erro_rel)
    ...
    plt.tight_layout()
    plt.show()
print("Erro relativo:", sum(err)/len(err))